...
`ApplicationConfig` є canonical-конфігурацією runtime-рівня застосунку DP1.

У межах поточної картки визначено тільки секцію `logging`. Інші секції
конфігурації runtime-рівня застосунку навмисно не визначаються в цій задачі та
мають бути описані окремими майбутніми canonical-картками або майбутніми
змінами цієї картки.

## Assumptions

- Повна модель `ApplicationConfig` ще не визначена.
- Поточна картка визначає тільки `ApplicationConfig.schema_version` і
...
У межах цієї картки визначено тільки:
