...
- Replaced the previous Python backend direction with a PHP Laravel backend in `currency-dashboard/` after the user requested Laravel.
...
- Added a Vue 3 frontend in `currency-dashboard/resources/frontend/` using Vue Router, Pinia and Axios from CDN builds.
- Added repository run instructions for the Laravel/Vue dashboard.
- Added task cards for traceability.
- Consolidated the deliverable into one downloadable `currency-dashboard/` folder.
...
- A Laravel server now serves both `/api/*` and the Vue static frontend from the same downloadable folder to keep local launch instructions simple.
