///Adaptation based on https://github.com/yan9a/serial 
// Quite poor quality. Needs to be rewritten
...
ceSerial::ceSerial(const std::string& Device, long BaudRate,long DataSize,char ParityType,float NStopBits):
	rxchar(0), 
	port(Device),
    stdbaud(true),
#ifdef CE_WINDOWS
...
int ceSerial::ReadBuff(char *data, int size) {
  if (!IsOpened()) {
    return 0;
  }
...
bool ceSerial::WriteChar(const char ch) {
  return write(fd, &ch, sizeof(ch)) == sizeof(ch);
}
