...
bool is_allowed_variant(const std::string_view stage, const std::string_view variant) {
    static const std::vector<std::string_view> acquisition{"passthrough"};
    ...
    if (stage == "acquisition") {
        return contains(acquisition, variant);
    }
    if (stage == "prep") {
...
dp1v2::PipelineStagesConfig parse_pipeline_stages(const json_t *stages_json) {
    reject_unknown_keys(stages_json,
                        {"acquisition", "prep", "radiometric", "enhancement", "matched_filter", "candidate_extraction",
                         "segmentation", "object_filtering", "measurement"},
                        "pipeline.pipeline");

    dp1v2::PipelineStagesConfig stages{};
    if (json_object_get(stages_json, "acquisition")) {
        stages.acquisition = parse_stage_config(stages_json, "acquisition");
    } else {
        stages.acquisition = dp1v2::StageConfig{
            .enabled = true,
            .variant = "passthrough",
            .level = "L0",
            .parameters = {},
